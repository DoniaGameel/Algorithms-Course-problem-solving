...
## Problem #6 Arrange Seats

Given a list of people's names (strings) of length N. You are required to print all the possible ways we can let those people sit next to each other such that no 2 people sitting next to each other having the same letter at the end of the first person’s name and at the beginning of the second person’s name (i.e john then natalia) (natalia then john is fine) You are required to answer the question above using brute force implemented using recursion.
...
ABC DEF ABD ABD ABC DEF DEF ABC ABD DEF ABD ABC
...
The second line contains N integers representing the scores of the first row.

The third line contains N integers representing the scores of the second row.

**Constraints**

N ranges between 0 and 10^6

The score of any square ranges from 0 to 10^8.

**Output Format**

The maximum possible total score the player can get.
...
The chosen squares are: 9, 8, 7, 5. Notice that we had to skip jumping on the squares of the column (5,1) in order to satisfy the jumping constraint.

## Problem #16 Train Breaking
